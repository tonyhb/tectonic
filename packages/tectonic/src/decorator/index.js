    ...
    class TectonicComponent extends PureComponent {
      ...
      componentWillReceiveProps(next) {
        if (typeof loadQueries === 'function' && next !== undefined) {
          ...
          // We only need to add queries generated by a dependency tree to
          // this.queries if it has new parameters; otherwise we end up
          // re-requesting items endlessly.
          Object.keys(newQueries).forEach((q) => {
            if (this.queries[q] === undefined) {
              // In this case a query has new parmaeters based on previously
              // loaded data, which needs to be added to this.queries so it's
              // resolved and props are added during render().
